...
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><%
	...
	try {
		...
		arrEmt = home.getMainEmt(conn, gubun, "%"+cate+"%", 0, 20);
		arrCate = home.getMainCate(conn, gubun);
		
		arrEmtlms = home.getMainLMS(conn, gubunlms, "%"+catelms+"%", 0, 15);
...
<div id="main"><!--main Start-->
        ...
        <fieldset id="emoticon">
            <ul class="title">
                <li class="<%=(gubun.equals("업종별문자"))?"mainbusinessover":"mainbusiness" %>" onclick="window.location.href='?gubun=업종별문자<%="&"+urllms+"&"+urlmms%>'">업종별문자</li>
				<li class="<%=(gubun.equals("테마문자"))?"mainthemaover":"mainthema" %>" onclick="window.location.href='?gubun=테마문자<%="&"+urllms+"&"+urlmms%>'">테마별문자</li>
            ...
            <div class="middle">
                ...
                <div class="emtibox">
                    ...
                    <textarea class="emti" onclick="setMsg(this.value)" readonly><%=arrEmt[15] %></textarea>
                    <textarea class="emti" onclick="setMsg(this.value)" readonly><%=arrEmt[16] %></textarea>
                    <textarea class="emti" onclick="setMsg(this.value)" readonly><%=arrEmt[17] %></textarea>
                    <textarea class="emti" onclick="setMsg(this.value)" readonly><%=arrEmt[18] %></textarea>
                    <textarea class="emti" onclick="setMsg(this.value)" readonly><%=arrEmt[19] %></textarea>
                </div>
            </div>
        </fieldset>
        ...
        <fieldset id="emoticon">
            <ul class="title">
				<li class="<%=(gubunlms.equals("업종별문자"))?"mainbusinessover":"mainbusiness" %>" onclick="window.location.href='?gubunlms=업종별문자<%="&"+url+"&"+urlmms%>'">업종별문자</li>
                <li class="<%=(gubunlms.equals("테마문자"))?"mainthemaover":"mainthema" %>" onclick="window.location.href='?gubunlms=테마문자<%="&"+url+"&"+urlmms%>'">테마별문자</li>
            ...
            <div class="middle">
               <div class="subTitle"><%
                ...
                <div class="emtibox">
                    ...
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[9] %></textarea><p style="height:10px"></p>
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[10] %></textarea>
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[11] %></textarea>
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[12] %></textarea>
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[13] %></textarea>
                    <textarea class="emtiLms" onclick="setMsg(this.value)" readonly><%=arrEmtlms[14] %></textarea>
                </div>
            </div>
        </fieldset>
            ...
            <% 
            	if (arrMms != null && arrMms.size() > 0) {
            		...
            		%><div style="float:left;width:180px;text-align:center;">
							<img onclick="setPhoto('<%= SLibrary.IfNull(hm, "msg") %>')" src="<%= SLibrary.IfNull(hm, "msg") %>" class="potoimg" style="display:block;width:120px;height:160px;cursor:pointer" <%= m == (arrMms.size() -1) ? "style='margin-right:0px;'" : "" %> />
