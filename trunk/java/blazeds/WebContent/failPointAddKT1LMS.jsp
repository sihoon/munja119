...
<%
	...
	try {
		...
		connSMS = VbyP.getDB("kt1");
		sql.append("select C.USER_ID, count(*) as CNT from (select A.USER_ID from SDK_MMS_REPORT A inner join SDK_MMS_REPORT_DETAIL B on(A.MSG_ID = B.MSG_ID) where A.SEND_DATE like ? and A.CONTENT_COUNT =0 and B.RESULT  != 2 AND A.USER_ID!='2jaegyoon' union all select USER_ID from SDK_MMS_SEND where SEND_DATE like ? and CONTENT_COUNT =0 and SEND_RESULT  in ('98','99') and USER_ID!='2jaegyoon' ) C  GROUP BY C.USER_ID");
		...
		if (count > 0) {
			...
			for (int i = 0; i < count; i++) {
				...
				pm.insertUserPoint(conn, mvo, 47, SLibrary.intValue( SLibrary.IfNull(hm, "CNT") ) * PointManager.DEFULT_POINT*3);
